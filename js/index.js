...
* 参数：切换间隔时间（以秒为单位，最小值为2秒）
**/
function slideImgBanenr(second) {
	// 根据图片数量添加控制点
	banner.prepend("<point class='point'></point>");
	var pointBox = $("point.point");
	for(var i = 0; i < imgLen; i++) {
	...
	// 添加过渡效果
	banner.children("[class^='img-']").css("transition","1.6s cubic-bezier(0, 0, .25, .99)");
	// 控制点位置计数器
	var count = 0;
	// 如果时间间隔参数小于2或未定义
	...
	setInterval(function() {
		...
		setTimeout(function(){
			...
			banner.children("[class^='img-']").eq(imgLen - 1).css({
				...
				"transition": "1.6s cubic-bezier(0, 0, .25, .99)"
			});
		},1600);
	}, second * 1000);
}
